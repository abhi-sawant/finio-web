...
export const api = {
  ...
  verifyOtp: (email: string, otp: string) =>
    apiFetch<LoginResult>('/auth/verify-otp', { method: 'POST', body: JSON.stringify({ email, otp }) }),
  ...
  listBackups: (token: string) =>
    apiFetch<{ backups: Array<{ backup_date: string; file_size: number; created_at: string }> }>('/backup/list', { method: 'GET', token }),

  getBackup: (token: string, date: string) =>
    apiFetch<Record<string, unknown>>(`/backup/${encodeURIComponent(date)}`, { method: 'GET', token }),

  deleteBackup: (token: string, date: string) =>
    apiFetch(`/backup/${encodeURIComponent(date)}`, { method: 'DELETE', token }),

  getProfile: (token: string) =>
    apiFetch<{ user: AuthUser }>('/user/me', { method: 'GET', token }),

  updateProfile: (token: string, data: { name?: string; current_password?: string; new_password?: string }) =>
    apiFetch<{ token: string; user: AuthUser }>('/user/me', { method: 'PUT', token, body: JSON.stringify(data) }),
